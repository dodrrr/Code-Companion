      ...
      >
        ...
        {!isToday && items.length > 0 && (
          <View style={[styles.tomorrowSet, { backgroundColor: colors.primary + '12', borderColor: colors.primary + '55' }]}>
            ...
            <View style={styles.tomorrowSetCopy}><Text style={[styles.tomorrowSetTitle, { color: colors.foreground }]}>Tomorrow is set</Text><Text style={[styles.tomorrowSetBody, { color: colors.mutedForeground }]}>{items.length} tasks · {priorityItem ? `One thing: ${priorityItem.text}` : 'Choose one thing that matters most.'}</Text></View>
          </View>
        )}
...
function DayReviewMoment({ visible, completedCount, totalCount, pendingItems, onMove, onLetGo, onClose, onPrepareTomorrow }: { visible: boolean; completedCount: number; totalCount: number; pendingItems: PlanItem[]; onMove: (item: PlanItem) => void; onLetGo: (item: PlanItem) => void; onClose: () => void; onPrepareTomorrow: () => void }) {
  const colors = useColors();
