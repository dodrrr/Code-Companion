export const CHAIN_COLORS = [
  '#FF6B35', // orange
  '#00C896', // emerald
  '#A855F7', // violet
  '#F43F5E', // rose
  '#F59E0B', // amber
  '#3B82F6', // blue
];

export const EXTRA_CHAIN_COLORS = [
  '#FBBF24', // gold
  '#84CC16', // lime
  '#22D3EE', // cyan
  '#EF4444', // red
  '#4F46E5', // indigo
  '#14B8A6', // teal
  '#38BDF8', // sky
  '#818CF8', // periwinkle
  '#E879F9', // orchid
  '#FB7185', // coral
  '#A3E635', // spring
  '#94A3B8', // slate
];

export const CHAIN_COLOR_NAMES: Record<string, string> = {
  '#FF6B35': 'Orange', '#00C896': 'Emerald', '#A855F7': 'Violet', '#F43F5E': 'Rose',
  '#F59E0B': 'Amber', '#3B82F6': 'Blue', '#FBBF24': 'Gold', '#84CC16': 'Lime',
  '#22D3EE': 'Cyan', '#EF4444': 'Red', '#4F46E5': 'Indigo', '#14B8A6': 'Teal',
  '#38BDF8': 'Sky', '#818CF8': 'Periwinkle', '#E879F9': 'Orchid', '#FB7185': 'Coral',
  '#A3E635': 'Spring', '#94A3B8': 'Slate',
};

const palette = {
  text: '#F5F5F7',
  tint: '#FF6B35',
  background: '#060607',
  foreground: '#F5F5F7',
  card: 'rgba(18,18,20,0.94)',
  cardSolid: '#121214',
  cardForeground: '#F5F5F7',
  primary: '#FF6B35',
  primaryForeground: '#FFFFFF',
  secondary: 'rgba(28,28,31,0.82)',
  secondaryForeground: '#F5F5F7',
  muted: 'rgba(30,30,33,0.72)',
  mutedForeground: '#8E8E95',
  accent: '#FF6B35',
  accentForeground: '#FFFFFF',
  destructive: '#FF3B30',
  destructiveForeground: '#FFFFFF',
  border: 'rgba(255,255,255,0.075)',
  input: 'rgba(255,255,255,0.095)',
  glass: 'rgba(16,16,18,0.78)',
  glassStrong: 'rgba(22,22,24,0.88)',
  surfaceRaised: '#17171A',
  glassBorder: 'rgba(255,255,255,0.09)',
  glassHighlight: 'rgba(255,255,255,0.05)',
};

const colors = {
  light: palette,
  dark: palette,
  radius: 22,
};

export default colors;
